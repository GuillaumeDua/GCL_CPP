...
namespace GCL
{
	namespace Experimental
	{
		...
		namespace Puzzle4
		{
			template <typename T_Abstract>
			struct UsingInterface
			{
				template <typename T>
				struct TypeIDToCTorPair
				{
					static const std::pair<int, std::function<T_Abstract*()>> value;
				};
				using TypeIdToCtorMap = std::unordered_map<size_t, std::function<T_Abstract*()>>;

				template <typename ... T_Childrens>
				static TypeIdToCtorMap Make_typeIdToCtorMap(void)
				{
					return { TypeIDToCTorPair<T_Childrens>::value... };
				}

				template <typename ... T_Childrens>
				struct ForEach
				{
					static TypeIdToCtorMap typeIdToCtorMap;
				};
			};

			template <typename T_Abstract>
			template <typename T>
			const std::pair<int, std::function<T_Abstract*()>> UsingInterface<T_Abstract>::TypeIDToCTorPair<T>::value =
			{
				GCL::TypeTrait::TypeToUniqueId<T>::value,
				[]() ->  T_Abstract* { return new T(); }
			};

			// TODO : FixMe : Double parameter pack expand
			template <typename T_Abstract>
			template <typename ... T_Childrens>
			typename UsingInterface<T_Abstract>::TypeIdToCtorMap	UsingInterface<T_Abstract>::ForEach<T_Childrens...>::typeIdToCtorMap;// = UsingInterface<T_Abstract>::Make_typeIdToCtorMap<T_Childrens...>();

			struct Test
			{
				static bool Proceed(void)
				{
					struct MyInterface { virtual const std::string name() = 0; };
					struct A : MyInterface { const std::string name() { return "A"; } };
					struct B : MyInterface { const std::string name() { return "B"; } };
					struct C : MyInterface { const std::string name() { return "C"; } };
					struct D : MyInterface { const std::string name() { return "D"; } };

					//std::cout << "size=[" << UsingInterface<MyInterface>::ForEach<A, B, C>::typeIdToCtorMap.size() << ']' << std::endl;
					//for (auto & elem : UsingInterface<MyInterface>::ForEach<A, B, C>::typeIdToCtorMap)
					//	std::cout << elem.first << " => " << elem.second()->name() << std::endl;

					std::cout << "size=[" << UsingInterface<MyInterface>::Make_typeIdToCtorMap<A, B, C>().size() << ']' << std::endl;
					for (auto & elem : UsingInterface<MyInterface>::Make_typeIdToCtorMap<A, B, C>())
						std::cout << elem.first << " => " << elem.second()->name() << std::endl;

					UsingInterface<MyInterface>::TypeIdToCtorMap map = UsingInterface<MyInterface>::Make_typeIdToCtorMap<A, B, C>();
