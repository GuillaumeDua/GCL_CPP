...
namespace GCL
{
	struct	ThreadSafeStdCout
	{
		ThreadSafeStdCout() = default;
		ThreadSafeStdCout(const ThreadSafeStdCout &) = delete;
		ThreadSafeStdCout(const ThreadSafeStdCout &&) = delete;
		ThreadSafeStdCout & operator=(const ThreadSafeStdCout &) = delete;
		~ThreadSafeStdCout() = default;
		...
		template <typename T>
		ThreadSafeStdCout &	operator<<(const T & data)
		{
			{
				std::lock_guard<std::mutex>	lockGuard(this->_mutex);
				std::cout << data;
	...
	struct	Consumer
	{
		...
		void		Work(void)
		{
			tscout << "[+] : Consumer : Work starting"/* << std::endl*/;
