...
namespace gcl::pattern::ecs
{
    ...
    struct components { // MP helper for component types
    ...
    template <typename... ts_components>
    struct entity { // todo : private members, friendship with manager ?
    ...
    template <typename... ts_components>
    struct components_storage { // per-component contiguous storage
        ...
        static_assert(
            not std::disjunction_v<std::is_reference<ts_components>...>); // refs are not default-constructible
        ...
        {
            gcl::tuple_utils::for_each(content, [&size](auto& value) { value.resize(size); });
        }
        void reserve(std::size_t size)
        {
            gcl::tuple_utils::for_each(content, [&size](auto& value) { value.reserve(size); });
        }
        auto size() const
    ...
    template <typename... ts_components>
    struct contract { // establish a contract. e.g a requierement
    ...
    template <typename... ts_components>
    struct manager { // manage an entitiy collection
        ...
        manager(std::size_t initial_capacity) { reserve_capacity(std::max<decltype(capacity())>(initial_capacity, 1)); }
        ...
        { // create a new initialized entity
            ...
            using return_type = std::tuple<id_type, persistent_id_type>;
        ...
        { // reference may point to the wrong element after reordering
            ...
            using return_type = std::tuple<entity_type&, std::tuple<requiered_components&...>>;
            return return_type{
                entity, std::tuple<requiered_components&...>{entity_add_component<requiered_components>(entity.id)...}};
        }
        ...
        constexpr decltype(auto) create_entity_impl(requiered_components_args&&... args)
        {
            ...
            return return_type{
                entity,
                std::tuple<requiered_components&...>{entity_add_component<requiered_components>(
                    entity.id, std::forward<requiered_components_args>(args))...}};
        ...
        constexpr decltype(auto) create_entity(tuple_type<requiered_components_args...>&& args_as_tuple)
        {
            return std::apply(
                [this](requiered_components_args... args) {
                    return create_entity_impl<requiered_components...>(
                        std::forward<requiered_components_args>(args)...);
                },
        ...
        auto entities_count() const { return entity_creation_index; }
        void clear() noexcept
        {
        ...
        bool entity_has_components(id_type id) const
        {
            ...
            static constexpr auto filter = gcl::mp::filter::as_bitset(
                components_type::as_type_pack{}, gcl::mp::type_pack<requiered_components...>{});
        ...
        const auto& get_entities() const { return entities; }
      ...
      private:
        ...
        auto create_entity_fwd_tuple_impl(
            std::tuple<requiered_components_args&&...>&& args_as_forwarded_tuple, std::index_sequence<indexes...>)
        {
